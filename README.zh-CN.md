...
> ✨ **v0.2.7（最新）：** **维基链接**与**反向链接面板**。**Vim 模式**。首次启动**欢迎页**。**GitHub 风格提示框**。**Ctrl+滚轮缩放**。**格式化后保持选中**。**前言编辑器**。预览中渲染图片。**格式工具栏**与**侧边面板切换**。**Nix/NixOS flake 支持**。新安装包：**.deb**、**.rpm**、macOS **.dmg** 与 **.app** 包。**单实例**文件打开。20+ 问题修复。详见 [CHANGELOG.md](CHANGELOG.md)。
