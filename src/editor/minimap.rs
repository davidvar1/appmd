...
use eframe::egui::{self, Color32, FontId, Pos2, Rect, Sense, Stroke, StrokeKind, Ui, Vec2};
...
impl<'a> Minimap<'a> {
    ...
    pub fn show(self, ui: &mut Ui) -> MinimapOutput {
        ...
        let is_dark = self
            ...
            .map(|c| c.is_dark())
            .unwrap_or(false);
        let ferrite_accent = self
            .theme_colors
            .as_ref()
            .map(|c| c.editor.heading)
            .unwrap_or_else(|| crate::theme::accent::default_accent());
        let colors = MinimapColors::new(is_dark, ferrite_accent);
        ...
        if let Some(vp_rect) = viewport_rect {
            ...
            painter.rect_stroke(
                vp_rect,
                2.0,
                Stroke::new(1.0, colors.viewport_border),
                StrokeKind::Inside,
            );
        }
...
impl<'a> SemanticMinimap<'a> {
    ...
    pub fn show(self, ui: &mut Ui) -> SemanticMinimapOutput {
        ...
        let ferrite_accent = self
            ...
            .map(|c| c.editor.heading)
            .unwrap_or_else(|| crate::theme::accent::default_accent());
        let colors = SemanticMinimapColors::new(is_dark, ferrite_accent);
                    ...
                    let (font_size, indent, label_text, label_color) = match item.content_type {
                        ContentType::Heading(level) => {
                            ...
                            let col = if is_current {
                                ...
                                heading_level_color_semantic(level, is_dark, ferrite_accent)
                            };
                            (fs, ind, lbl, col)
                        }
...
fn heading_level_color_semantic(level: u8, is_dark: bool, accent: Color32) -> Color32 {
    use crate::theme::accent;
    if is_dark {
        match level {
            1 => accent,
            2 => Color32::from_rgb(150, 220, 150),
            3 => Color32::from_rgb(220, 180, 120),
            4 => Color32::from_rgb(200, 150, 200),
            5 => Color32::from_rgb(180, 180, 180),
            _ => Color32::from_rgb(150, 150, 150),
        }
    } else {
        match level {
            1 => accent::lerp_color(accent, Color32::BLACK, 0.08),
            2 => Color32::from_rgb(50, 140, 50),
            3 => Color32::from_rgb(180, 120, 40),
            4 => Color32::from_rgb(140, 80, 140),
            5 => Color32::from_rgb(100, 100, 100),
...
impl SemanticMinimapColors {
    fn new(is_dark: bool, accent: Color32) -> Self {
        use crate::theme::accent;
        if is_dark {
            Self {
                ...
                current_section_bg: accent::lerp_color(Color32::from_rgb(25, 25, 25), accent, 0.4),
                current_level_indicator: accent,
                hover_bg: Color32::from_rgb(40, 40, 45),
                code_block: Color32::from_rgb(180, 140, 220),
                mermaid_diagram: Color32::from_rgb(100, 200, 180),
                table: Color32::from_rgb(220, 180, 100),
                image: Color32::from_rgb(200, 130, 160),
                blockquote: Color32::from_rgb(140, 160, 200),
            ...
            Self {
                ...
                current_section_bg: accent::lerp_color(
                    Color32::from_rgb(248, 248, 248),
                    accent,
                    0.24,
                ),
                current_level_indicator: accent::lerp_color(accent, Color32::BLACK, 0.06),
                hover_bg: Color32::from_rgb(235, 235, 240),
                code_block: Color32::from_rgb(130, 80, 170),
                mermaid_diagram: Color32::from_rgb(50, 140, 120),
                table: Color32::from_rgb(180, 130, 40),
                image: Color32::from_rgb(160, 80, 120),
                blockquote: Color32::from_rgb(80, 100, 160),
...
impl MinimapColors {
    fn new(is_dark: bool, accent: Color32) -> Self {
        use crate::theme::accent;
        let link = accent::standard_link_color(is_dark);
        if is_dark {
            Self {
                background: Color32::from_rgb(25, 25, 25),
                border: Color32::from_rgb(50, 50, 50),
                text: Color32::from_rgba_unmultiplied(180, 180, 180, 200),
                heading: Color32::from_rgba_unmultiplied(accent.r(), accent.g(), accent.b(), 220),
...
mod tests {
    ...
    fn test_minimap_colors_dark() {
        let colors = MinimapColors::new(true, crate::theme::accent::default_accent());
    ...
    fn test_minimap_colors_light() {
        let colors = MinimapColors::new(false, crate::theme::accent::default_accent());
    ...
    fn test_heading_level_color_semantic_dark() {
        ...
        let colors: Vec<_> = (1..=6)
            .map(|l| heading_level_color_semantic(l, true, crate::theme::accent::default_accent()))
            .collect();
    ...
    fn test_heading_level_color_semantic_light() {
        // Verify each level returns a different color
        let colors: Vec<_> = (1..=6)
            .map(|l| heading_level_color_semantic(l, false, crate::theme::accent::default_accent()))
            .collect();
    ...
    fn test_semantic_minimap_colors_light() {
        let colors = SemanticMinimapColors::new(false, crate::theme::accent::default_accent());
        // Light theme should have light background
        assert!(colors.background.r() > 200);
