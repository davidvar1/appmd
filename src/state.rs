...
impl FoldKind {
    ...
    pub fn icon(&self) -> &'static str {
        match self {
            FoldKind::Heading(_) => "Â§",
            FoldKind::CodeBlock => "{ }",
            FoldKind::List => "â€¢",
            FoldKind::Indentation => "â¤µ",
        }
    }
}
...
pub struct ImageViewerState {
    ...
    pub zoom: f32,
    /// Image dimensions (width, height) â€” populated after first load
...
/// Content state for a tab â€” either loading in background or fully loaded.
#[derive(Debug, Clone)]
pub enum TabContent {
...
pub struct Tab {
    ...
    // â”€â”€ Per-frame cache fields (invalidated via content_version) â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
impl Tab {
    ...
    /// Toggle the view mode: Raw â†’ Split â†’ Rendered â†’ Raw
...
///
/// For small workspaces (â‰¤50 files), backlinks are scanned on demand when
...
pub struct BacklinkIndex {
    /// Map from lowercase filename (without extension) â†’ list of files that link to it.
...
/// Page editor state (block editing buffers, focus, etc.)
pub page_editor: PageEditorState,
}

/// State for the Notion-like page block editor.
/// Maintains per-block text editing buffers and focus tracking.
#[derive(Debug)]
pub struct PageEditorState {
    /// Per-block text editing buffers (block_id -> current text)
    pub buffers: std::collections::HashMap<String, String>,
    /// The block ID that currently has keyboard focus
...
impl PageEditorState {
    pub fn get_buffer(&mut self, block_id: &str, initial: &str) -> &mut String {
        self.buffers
            .entry(block_id.to_string())
            .or_insert_with(|| initial.to_string())
    }

    pub fn sync_buffer(&mut self, block_id: &str, content: &str) {
        self.buffers
            .insert(block_id.to_string(), content.to_string());
    }

    pub fn remove_buffer(&mut self, block_id: &str) {
...
impl AppState {
    ...
    pub fn show_welcome_tab(&mut self) {
        ...
        // Otherwise create it (this should also set active, but weâ€™ll be safe)
    ...
    pub fn open_file_with_focus(
        ...
        // Intercept image files before binary detection â€” open as image viewer
        if FileType::from_path(&path).is_image() {
            return self.open_image_tab(path, focus);
        }

        // Intercept PDF files before binary detection â€” open as PDF viewer
    ...
    fn open_page_store() -> Option<SqliteStore> {
        let data_dir = dirs::data_dir().map(|d| d.join("AppMD"));
        match data_dir {
            Some(dir) => {
...
mod tests {
    ...
    fn test_binary_detection_text() {
        ...
        assert!(!is_binary_content("Special chars: Ã¤Ã¶Ã¼ Ã©moji ðŸŽ‰".as_bytes()));
    }
    ...
    fn test_tab_should_prompt_to_save() {
        ...
        // Quick note: pathless modified â€” no save prompt on close/exit
